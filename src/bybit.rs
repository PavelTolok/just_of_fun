        ...
        while let Some(message) = ws_stream.next().await {
            match message {
                Ok(Message::Text(text)) => match serde_json::from_str::<BybitWsResponse>(&text) {
                    Ok(parse_msg) => {
                        // Пропускаем сообщения без topic (например, подтверждения подписки)
                        if let Some(topic) = parse_msg.topic {
                            let symbol = topic.split(".").last().unwrap().to_string();
                            if common_tickers.contains(&symbol) && !parse_msg.data.is_empty() {
                                if let Ok(price) = parse_msg.data[0].close.parse::<f64>() {
                                    {
