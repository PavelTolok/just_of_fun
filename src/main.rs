...
async fn main() {
    ...
    // Режим: вывести все TELEGRAM_CHAT_ID пользователей и завершиться.
    if env::var("PRINT_TELEGRAM_CHAT_IDS").is_ok() {
        if let Err(e) = crate::telegram::log_all_chat_ids_from_telegram().await {
            eprintln!("Ошибка при получении TELEGRAM_CHAT_ID: {}", e);
        }
        return;
    }
